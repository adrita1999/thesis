...
namespace kmdiff {
               ...
               std::shared_ptr<Sampler<DMAX_C>> sampler)
  {
    ...
    //modified
    spdlog::info("Starting phenotype file read...");

    int noSamples;
    noSamples = opt->nb_controls + opt->nb_cases;

    double *phenotypeVals;
    phenotypeVals= new double[noSamples];

    FILE *phenotypeFile = fopen("test.pheno", "r");

    for (int i = 0; i < noSamples; i++)
    {
      int dump = fscanf(phenotypeFile, "%lf\n", &phenotypeVals[i]);
    }
    ...
    // for (int i = 0; i < noSamples; i++)
    // {
    //   std::cout<<phenotypeVals[i]<<"  "<<&phenotypeVals[i]<<endl;
    // }

    spdlog::info("Phenotype file read done...");
    

    if (opt->model_lib_path.empty())
    {
      model = std::make_shared<PoissonLikelihood<DMAX_C>>(
        opt->nb_controls, opt->nb_cases, total_controls, total_cases, opt->log_size, phenotypeVals);
    }

    model->computeNullLikelihood();
    //
